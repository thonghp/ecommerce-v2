...
$(document).ready(function () {
    productDetailCount = $(".hiddenProductId").length; // xử lý khi nhấn nút tăng giảm số lượng sản phẩm sẽ update giá
    $("#products").on("click", "#linkAddProduct", function (e) {
        e.preventDefault();
        link = $(this);
        url = link.attr("href");

        //  handle show modal when pressing button add product
...
// Check if the selected product exists in the order
...
function getShippingCost(productId) {
    ...
    // xử lý đọc về state và city từ mục shipping đang chọn
    state = $("#state").val();
    if (state.length === 0) {
        state = $("#city").val();
    }
...
// code hiển thị sản phẩm khi thêm vào đơn hàng
...
function generateProductCode(productId, productName, mainImagePath, productCost, productPrice, shippingCost) {
    // xử lý khi nhấn nút tăng giảm số lượng sản phẩm sẽ update giá tạm tính đúng vị trí sản phẩm đó
		...
		<div class="border rounded p-1" id="${rowId}">
			...
			<div class="row">
				<div class="col-1">
					...
					<div><a class="fas fa-trash icon-gray linkRemove" href="" rowNumber="${nextCount}"></a></div>				
				</div>
				<div class="col-3">
			...
			<table>
				...
				<tr>
					<td>Shipping Cost:</td>
					<td>
						<input type="text" required class="form-control m-1 ship-input" name="productShipCost"
